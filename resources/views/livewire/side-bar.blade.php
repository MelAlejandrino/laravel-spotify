<div class="hidden md:flex flex-col w-64 bg-zinc-900 p-4">
    <div class="mb-8">
        <h1>{{ $spotifyUser['name'] }}</h1>
    </div>
    <nav class="space-y-1">
        <a href="/"
            class="flex items-center px-4 py-2 text-sm font-medium rounded-md bg-zinc-800 text-white">
            <svg class="mr-3 h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                <path
                    d="M12.5 3.247a1 1 0 0 0-1 0L4 7.577V20h4.5v-6a1 1 0 0 1 1-1h5a1 1 0 0 1 1 1v6H20V7.577l-7.5-4.33zm-2-1.732a3 3 0 0 1 3 0l7.5 4.33a2 2 0 0 1 1 1.732V21a1 1 0 0 1-1 1h-6.5a1 1 0 0 1-1-1v-6h-3v6a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V7.577a2 2 0 0 1 1-1.732l7.5-4.33z">
                </path>
            </svg>
            Home
        </a>
    </nav>
</div>